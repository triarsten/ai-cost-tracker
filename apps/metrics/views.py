...
def ai_metrics(request):
    ...
    rows_month = (
        CostRecord.objects.filter(date__gte=month_start).values("provider__name").annotate(total_cost=Sum("cost_usd"))
    )
    for row in rows_month:
        month_cost_gauge.labels(provider=row["provider__name"]).set(float(row["total_cost"] or 0))

    last_syncs = (
        SyncLog.objects.filter(status="success").values("provider__name").annotate(last_sync=Max("finished_at"))
    )
